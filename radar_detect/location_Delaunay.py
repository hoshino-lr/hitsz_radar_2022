...
class location_Delaunay(object):
    ...
    def __init__(self, cam_side: str, debug: bool = False, rvec=None, tvec=None):
        ...
        self.rect_points = np.array([
            [self.cam_rect[0], self.cam_rect[0]],
            [self.cam_rect[0] + self.cam_rect[2], self.cam_rect[0]],
            [self.cam_rect[0], self.cam_rect[1] + self.cam_rect[3]],
            [self.cam_rect[0] + self.cam_rect[2], self.cam_rect[1] + self.cam_rect[3]]
        ])
    ...
    def _get_region(self):
        ...
        self.w_points = self.c_points[rows]
    ...
    def get_point_pos(self, l: np.ndarray):
        if not self.init_ok:
            return np.ndarray((3,), dtype=np.nan)
        if l.shape[1] == 4:
            res = self.Dly.locate(tuple(l.shape[1:]))
            value = res[0]
            ...
            if value == cv.SUBDIV2D_PTLOC_INSIDE:
                ...
                w_point = self._cal_pos_triangle(
                    np.array([self.Dly.edgeOrg(first_edge),
                              self.Dly.edgeOrg(second_edge),
                              self.Dly.edgeOrg(third_edge),
                              ]),
                    l[1:]
                )
    ...
    def _cal_pos_triangle(self, pts: np.ndarray, pt) -> np.ndarray:
        if self._check(pts):
            ...
            magnitude3 = pt - pts[0]
            k_1 = np.dot(magnitude1, magnitude3) / self._mag_pow(magnitude1)
            k_2 = np.dot(magnitude2, magnitude3) / self._mag_pow(magnitude2)
            h_1 = self.w_points(np.where(self.cam_points == pts[0]))
            h_2 = self.w_points(np.where(self.cam_points == pts[1]))
            h_3 = self.w_points(np.where(self.cam_points == pts[2]))
    ...
    def _cal_pos_vertex(self, pts) -> np.ndarray:
        if self._check(pts):
            ...
            return self.w_points(np.where(self.cam_points == pts))

    def get_points_pos(self, t_location):
        pass

    def _check(self, pts: np.ndarray) -> bool:
        for i in pts:
            res = i == self.cam_points
