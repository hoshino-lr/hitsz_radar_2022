...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    if pretrained:
        ...
        if hyp.get('net1_anchors'):
            ckpt['model'].yaml['net1_anchors'] = round(hyp['net1_anchors'])  # force autoanchor
        model = Model(opt.cfg or ckpt['model'].yaml, ch=3, nc=nc).to(device)  # create
        exclude = ['anchor'] if opt.cfg or hyp.get('net1_anchors') else []  # exclude keys
    ...
    # Image sizes
    gs = int(model.stride.max())  # net1_grid size (max stride)
...
if __name__ == '__main__':
    ...
    # Evolve hyperparameters (optional)
        ...
        meta = {'lr0': (1, 1e-5, 1e-1),  # initial learning rate (SGD=1E-2, Adam=1E-3)
                ...
                'net1_anchors': (2, 2.0, 10.0),  # net1_anchors per output net1_grid (0 to ignore)
